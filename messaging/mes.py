...
def reqRow(num):
    try:
        personLink = requests.get(f"http://127.0.0.1:5000/getRawRow?table=peoplelinks&stRow={num}&nums=1")
...
def run(driver):
    currentPersonNum = 250
    # 650 hogay tha
    tem = requests.get("http://127.0.0.1:5000/tableSize?tableName=peoplelinks")
    tem = json.loads(tem.content)
    tem = tem[0][4]
    print("in here",currentPersonNum,tem)
    while currentPersonNum < tem:
        try:
            ...
            if conn == 'None': 
                ...
                time.sleep(1)
                app[win[pos].title].set_focus()
                pyautogui.moveTo(int(win[pos].topleft[0])+int(locat['x'])+100 , int(win[pos].topleft[1])+int(locat['y'])+500)
                pyautogui.click()
            ...
            file = open("somethingWentWrong.txt","a")
            file.write(f"\n{link}")
            file.close()
        ...
        print("currentPersonNum - ",currentPersonNum)
